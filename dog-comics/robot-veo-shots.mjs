// 🐶 VEO POR TOMAS (versión C) — anima cada toma del storyboard con Veo 3.1 (lip-sync real).
// Veo genera SU PROPIA voz (no usa ElevenLabs). La imagen es el primer cuadro.
//
// Uso:  node dog-comics/robot-veo-shots.mjs give-me-the-paw 05,13,20   (sample)
//       node dog-comics/robot-veo-shots.mjs give-me-the-paw            (todas)
// OJO COSTO (fal.ai): ~$0.40–0.60 por toma. Idempotente.
...
async function veo(input) {
  const sub = await fetch(`https://queue.fal.run/${MODEL}`, { method: 'POST', headers, body: JSON.stringify(input) });
  if (!sub.ok) throw new Error(`encolar HTTP ${sub.status}: ${(await sub.text()).slice(0, 150)}`);
...
for (const shot of guion.shots) {
  ...
  const dice = shot.linea?.dice ? ` The character says, mouth lip-synced to the words: "${shot.linea.dice}".` : '';
  const prompt = `3D Pixar-style cartoon, keep the EXACT same characters and look from the image. ${accion}.${dice} Expressive comedic animation.`;
  console.log(`→ ${shot.id} (${shot.linea?.quien || 'escena'}) [Veo]: ${(shot.linea?.dice || accion).slice(0, 45)}... (tarda)`);
  try {
    const url = await veo({ prompt, image_url: dataUri, aspect_ratio: '9:16', duration: '4s', resolution: '1080p', generate_audio: true });
...
console.log(`\n📊 Veo: ${hechos} clips, ${saltados} ya existían.\n📁 output/${slug}/clips-C/\n`);
