...
async function loadGlossaryTerms() {
  ...
  if (entries.length === 0) {
    throw new Error(`No glossary entries parsed from ${GLOSSARY_TS} — formato del file cambió?`);
  }
  return entries;
}
...
// matchea "coffee" o similar. Lista mantenida explícitamente para que sea
// fácil agregar exclusiones futuras.
const GENERIC_SKIP_TERMS = new Set([
  // ej: 'coffee', 'water', 'machine' — todos demasiado genéricos.
]);
...
function buildMatchers(glossaryEntries) {
  ...
  for (const entry of glossaryEntries) {
    ...
    for (const variant of variants) {
      ...
      // o EOF — esto evita matches "dentro" de palabras como "espressoyield".
